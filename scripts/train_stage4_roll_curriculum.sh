...
if [[ $# -lt 2 ]]; then
  ...
  echo
  echo "Example:"
  echo "  $0 2026-07-30_XX-XX-XX_stable6d_stage3d_from_stage3c model_2700.pt"
  echo
...
train_stage "Isaac-WheeledLegged-Stage4a-v0" "stable6d_stage4a_from_stage3d" "${LOAD_RUN}" "${CHECKPOINT}"
train_stage "Isaac-WheeledLegged-Stage4b-v0" "stable6d_stage4b_from_stage4a" "${LOAD_RUN}" "${CHECKPOINT}"
train_stage "Isaac-WheeledLegged-Stage4c-v0" "stable6d_stage4c_from_stage4b" "${LOAD_RUN}" "${CHECKPOINT}"

echo
echo "Stage4 roll curriculum finished."
